...
### Использование RouterProvider loader
...
│   ├── core.ts                # Экспорт core функций
│   ├── react.ts               # Экспорт React компонентов
│   ├── core/
│   │   ├── types/
│   │   │   └── types.ts       # Типы плагина
│   │   ├── build-generator.ts # Генерация build файлов
│   │   ├── hmr-handlers.ts    # HMR обработчики
│   │   ├── manifest-loader.ts # Загрузка манифеста
│   │   ├── metadata.ts        # SEO метаданные
│   │   ├── route-generator.ts # Генерация маршрутов
│   │   ├── scanner.ts         # Сканирование файлов
│   │   ├── type-generator.ts  # Генерация типов
│   │   ├── utils.ts           # Утилиты
│   │   └── virtual-module.ts  # Виртуальный модуль
│   └── components/
│       ├── types/
│       │   └── types.ts       # React типы
│       ├── router-layout.tsx  # Layout компонент
│       ├── router-provider.tsx# Provider компонент
│       └── router-utils.tsx   # Утилиты роутера
